...
const BlogPage = () => {
  ...
  // Función para cargar noticias de ÉtherCode
  const fetchÉtherCodeNews = async () => {
    try {
      ...
      setÉtherCodeNews(articles);
    } catch (error) {
      console.error('Error fetching ÉtherCode news:', error);
    }
  };
  ...
  useEffect(() => {
    ...
    // Si también quieres cargar noticias de ÉtherCode, descomenta la siguiente línea:
    // fetchÉtherCodeNews();
  }, []);
  ...
  return (
    <>
      <Head>
        <title>Blog y Noticias | ÉtherCode</title>
        <meta name="description" content="Noticias de automatización con inteligencia artificial y experiencias reales desde ÉtherCode. Descubrí cómo mejorar tu negocio con tecnología." />
      </Head>
      <div className="bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100 min-h-screen">
        <Container>
          ...
          <section className="mb-12">
            <h2 className="text-3xl font-bold mb-6">Noticias de ÉtherCode</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {blogPosts.map((news) => (
                <NewsCardÉtherCode key={news.slug} news={news} />
              ))}
            </div>
          </section>
